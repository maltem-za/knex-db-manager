...
/**
 * Checks if the provided value has a 'seed' property that is a function.
 *
 * @param value
 * @returns {boolean}
 */
DatabaseManager.prototype.checkHasSeedFunction = function(value) {
  if (!value || !value.hasOwnProperty('seed')) {
    return false;
  }

  return typeof value.seed === 'function';
};
 ...
 * @returns {Promise}
 */
DatabaseManager.prototype.populateDb = async function(populatePathPattern) {
  populatePathPattern = populatePathPattern || this.config.dbManager.populatePathPattern;

  var knex = this.knexInstance();
  // stopped using _.isFunction as a parameter to filterModule here, partly because of: https://github.com/lodash/lodash/issues/3603
  var modules = multiRequire(populatePathPattern).filterModule(this.checkHasSeedFunction).require();

  for (var i=0; i<modules.length; i++) {
    await knex.transaction(function (trx) {
        return modules[i].module.seed(trx);
    });
  }
};
